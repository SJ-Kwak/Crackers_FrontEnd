...
import { CRACKER_API_URL } from 'react-native-dotenv';
import {
  setItemToAsync,
  getItemFromAsync,
  removeItemFromAsync,
...
export class Request {
  ...
  default = async (path, body) => {
    ...
    const url = CRACKER_API_URL + path;
    ...
    try {
      ...
      if (err.response == undefined) {
        ...
        console.log(err);
        alert("ERR_CONNECTION_REFUSED" + CRACKER_API_URL);
        console.warn(CRACKER_API_URL);
        throw err;
        ...
        try {
          const response = await axios.post(
            CRACKER_API_URL + "/accounts/token/refresh",
            {
