...
export default function MainDemo({ navigation }) {
  ...
  const getWorkSpace = async () => {
    ...
    if (!response.data[0]) {
      ...
      setDuringTime(
        (parseInt(response.data[0].schedules[0].endTime / 100) -
          parseInt(response.data[0].schedules[0].startTime / 100)) *
          60 +
          (parseInt(response.data[0].schedules[0].endTime % 100) -
            parseInt(response.data[0].schedules[0].startTime % 100)),
      );
  ...
  return (
    <View style={styles.container}>
      ...
      <ImageBackground
        source={require("../assets/BlurCircle.png")}
        style={styles.circle1}
        imageStyle={{ borderRadius: 280, backfaceVisibility: "visible" }}
        blurRadius={60}>
        {circlePo == -50 ? (
          <View>
            <Text
